...
def describe(name=None, dir=None, mode=None):
  proc = "describe"
      ...
      if not os.path.exists(path + ".inp"):  # get the data
        ...
        if method & 1:  # imputation
          ...
          if os.path.exists(path + ".agg"):  # remove the .agg file
            os.remove(path + ".agg")
        if not msg:
          rc = srclib.execute("describe", path, mode, None)  # execute describe
          if rc != 0:
            msg = "Abnormal termination of describe"
          else:
            if os.path.exists(path + ".out"):  # write the estimates
              f = open(path + ".out", "r")
              cmd = f.read()
              f.close()
              spss.Submit(cmd)  # execute the command
