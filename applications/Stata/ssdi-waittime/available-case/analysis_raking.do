/*
Example file for running analysis with RAKED available-case data and
...
svyset halfsamp [pw=finalinitwgt], strata(varstrat) brrweight(finalrepweight*) fay(0.5)
...
matrix SSDIoutputRake`k'=(beta,var)
matrix colnames SSDIoutputRake`k' = beta`k' var`k'
preserve
{
clear
set obs 2
gen N`k'=.
replace N`k'=`N'
svmat SSDIoutputRake`k', names(col)
save ${mydata}/SSDIoutputRake`k'.dta, replace
}
restore

reg adjud_wait1 ibn.female [pw=finalinitwgt], noc
local N=e(N)
matrix beta=e(b)'
matrix varcov=e(V)
matrix var=vecdiag(varcov)'
matrix SSDIoutputRakeNorepw`k'=(beta,var)
matrix colnames SSDIoutputRakeNorepw`k' = beta`k' var`k'
preserve
{
clear
set obs 2
gen N`k'=.
replace N`k'=`N'
svmat SSDIoutputRakeNorepw`k', names(col)
save ${mydata}/SSDIoutputRakeNorepw`k'.dta, replace
}
restore
...
local outcomes SSDIoutputRake SSDIoutputRakeNorepw
...
*/
use ${mydata}/FinalSSDIoutputRakeNorepw.dta, clear
append using ${mydata}/FinalSSDIoutputRake.dta, gen(Repw)
gen order=.
...
replace order=5 if Repw==1 & female==1
 ...
 twoway bar  Beta order if(female==0) ///
	...
	xtitle("") ytitle("Mean Wait Time (Months)") title("Adjudication Wait Time - Raking")

graph save Graph ${output}/GraphRake.gph, replace
