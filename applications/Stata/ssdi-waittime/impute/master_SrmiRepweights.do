...
/*  
This program serves as a shell file for using SRMI (via IVEware) to complete the data and
        create replicate weights. This program keeps the needed variables from the SSB/GSF 
...
The user must specify 3 things:
		...
		user programs ("programs"), dataset name, and number of replicates
...
STEPS:
        1) keep the SSB/GSF variables needed for the application
        2) call the SRMI program, which imputes the data and
                specifies post-imputation variable relationships
        3) call the replicate weight generation program


Input files: SSB/GSF data files
...
*/
***USER MUST EDIT THESE THREE LOCATIONS***
...
global replicates 4 /* "4" for synthetic data, "1" for itnernal GSF */
*Set IVEware use: path to srclib folder where IVEware commands are saved
global srclib "/cenwork/SSB/programs/v7.0/examples/evan/srclib/v3/srclib/stata"
...
keep personid male race hispanic foreign_born birthdate mbr_ssdi_ddo_* mbr_ssdi_dsd_* ///
	mbr_ssdi_dig_group_* educ_5cat state panel sipp_panel_beg_date initwgt halfsamp varstrat
keep if panel>=1990
