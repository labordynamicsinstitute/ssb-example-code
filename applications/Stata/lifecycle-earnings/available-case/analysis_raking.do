/*
Example program for running analysis with RAKED available-case data and
...
/*
USER MUST EDIT THE DATA AND OUTPUT PATHS BELOW AND THE DATA TYPE AND REPLICATES NUMBER MACROS
*/
...
forvalues i=1978/2011 {
        use total_der_fica* birthdate panel varstrat halfsamp initwgt* repweight* final* using ${mydata}/ssb_available_repw_reweight`k'.dta, clear
        keep if panel>=1990
        drop panel
...
svyset halfsamp [pw=finalinitwgt_reweight], strata(varstrat) brrweight(finalrepweight_reweight*) fay(0.5)
compress
...
*/
reg log_total_der_fica ibn.age [pw=finalinitwgt_reweight], noc
local N=e(N)
matrix beta=e(b)'
matrix varcov=e(V)
matrix var=vecdiag(varcov)'
matrix LCoutputRake`k'=(beta,var)
matrix colnames LCoutputRake`k' = beta`k' var`k'
preserve
{
clear
set obs 36
gen N`k'=.
replace N`k'=`N'
svmat LCoutputRake`k', names(col)
save ${mydata}/LCoutputRake`k'.dta, replace
}
restore
...
matrix varcov=e(V)
matrix var=vecdiag(varcov)'
matrix LCoutputRakeNorepw`k'=(beta,var)
matrix colnames LCoutputRakeNorepw`k' = beta`k' var`k'
preserve
{
clear
set obs 36
gen N`k'=.
replace N`k'=`N'
svmat LCoutputRakeNorepw`k', names(col)
save ${mydata}/LCoutputRakeNorepw`k'.dta, replace
}
restore
...
local outcomes LCoutputRake LCoutputRakeNorepw
...
use ${mydata}/FinalLCoutputRakeNorepw.dta, clear
keep Beta N CIUpper95 CILower95 age
foreach var in Beta N CIUpper95 CILower95 {
	rename `var' `var'Norepw
}
merge 1:1 age using ${mydata}/FinalLCoutputRake.dta, keepusing(Beta N CIUpper95 CILower95) gen(_mergeRepw)
...
title("Life-Cycle Earnings - Raking", size(medium)) ///
yscale(range(9 11)) ylabel(9(.5)11) xlabel(25(5)60)
graph save ${output}/GraphRaking.gph, replace
