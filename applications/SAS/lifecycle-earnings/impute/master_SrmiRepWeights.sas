...
The user must specify 2 things:
        1) paths to the input data and user data folder, dataset name, and number
		of replicates
...
/*/ SET PATHS, DATASET NAME, AND NUMBER OF SYNTHETIC/INTERNAL FILES /*/
...
*Set IVEware use: path to srclib folder where IVEware commands are saved and sasautos line;
options set = SRCLIB '/cenwork/SSB/programs/v7.0/examples/evan/srclib/v3/srclib/sas'
...
set inputs.&dataname.&k.(keep=personid male race hispanic foreign_born birthdate
        total_der_fica_: educ_5cat state panel
