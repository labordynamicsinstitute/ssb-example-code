...
/*/ 
Example file for running analysis with available case data that has been raked, 
	with and without replicate weights. The program performs the analysis separately over
        all four synthetic files, saves the results from each file, and combines
...
STEPS:
        ...
        6) round variables for output, then generate lifecycle earnings graph
...
proc sort data=ssb_available_repw_rake&k.;
by panel;
run;

data mydata.ssb_available_repw_rake_rew&k.;
merge ssb_available_repw_rake&k. mydata.relative_size;
by panel;
run;
...
data LCoutput_NorepwRake&k.(keep=Parameter Estimate&k. StdErr&k. Var&k.);
set LCoutput_NorepwRake&k.;
...
proc sort data=LCoutput_NorepwRake&k. out=LCoutput_NorepwRake&k.;
by Parameter;
run;
...
data LCoutput_RepwRake&k.(keep=Parameter Estimate&k. StdErr&k. Var&k.);
set LCoutput_RepwRake&k.;
rename Estimate=Estimate&k.;
rename StdErr=StdErr&k.;
Var=StdErr**2;
rename Var=Var&k.;
...
proc sort data=LCoutput_RepwRake&k. out=LCoutput_RakeRepw&k.;
by Parameter;
run;
...
data mydata.LCoutput_NorepwRake;
merge LCoutput_NorepwRake1 LCoutput_NorepwRake2 LCoutput_NorepwRake3 LCoutput_NorepwRake4;
by Parameter;
run;
%end;
...
%if &replicates.=4 %then %do;
data mydata.LCoutput_NorepwRake;
set mydata.LCoutput_NorepwRake;
...
* Merge outputs from each implicate;
%if &replicates.=4 %then %do;
data mydata.LCoutput_RepwRake;
merge LCoutput_RepwRake1 LCoutput_RepwRake2 LCoutput_RepwRake3 LCoutput_RepwRake4;
by Parameter;
run;
%end;
...
/*/ 6. ROUND VARIABLES, THEN GENERATE GRAPH /*/
*first, combine results with and without replicate weights into on dataset;
*generate variables in with replicate weights dataset for graph, then round;
data temp_LCoutput_RepwRake(keep=Parameter Age PointEstimate CIlower CIupper);
set mydata.LCoutput_RepwRake;
Age=substr(Parameter,5,2);
run;

data temp_LCoutput_RepwRake;
set temp_LCoutput_RepwRake;
...
proc sort data=temp_LCoutput_RepwRake out=temp_LCoutput_RepwRake;
by Age;
run;

*gnerate variables in without replicate weights dataset for graph, then round;
data temp_LCoutput_NorepwRake(keep=ParameterNorepw Age PointEstimateNorepw CIlowerNorepw CIupperNorepw);
set mydata.LCoutput_NorepwRake;
...
data temp_LCoutput_NorepwRake;
set temp_LCoutput_NorepwRake;
if int(PointEstimateNorepw) ne 0 then do;
    ...
    PointEstimateNorepw_4sigdigit=round(PointEstimateNorepw,10**(-1*(abs(int(log10(abs(PointEstimateNorepw))))
+4)));
    end;
...
proc sort data=temp_LCoutput_NorepwRake out=temp_LCoutput_NorepwRake;
by Age;
run;

data mydata.LCoutput_Rake_merged;
merge temp_LCoutput_RepwRake temp_LCoutput_NorepwRake;
by Age;
run;
...
ods graphics / reset=index imagename='GraphRaking' imagefmt=png;
ods listing gpath="&outpath.";


title "Life-Cycle Earnings - Raking";
proc sgplot data=mydata.LCoutput_Rake_merged noautolegend;
