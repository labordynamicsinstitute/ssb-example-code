...
/*/ 1. SET PATHS FOR SSB/GSF DATA AND USER DATA FILES /*/
...
%let replicates=4; *"4" for SSB, "1" for internal GSF;
...
set inputs.&dataname.&k.(keep=personid male race hispanic foreign_born birthdate
        total_der_fica_: educ_5cat state panel sipp_panel_beg_date initwgt halfsamp varstrat
	where=(panel>=1990));
run;
%end;
